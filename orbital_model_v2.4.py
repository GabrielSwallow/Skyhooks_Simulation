# -*- coding: utf-8 -*-
"""
Created on Sat Jun 20 20:19:26 2020
...
#%%
TIME_STEP = 6000 #time resolution in seconds
FRAMES=4200 #num frames in animation
FRAME_RATE = 100
...
def pathFinder(x0, y0, vx0, vy0):
    traj = spi.solve_ivp(dU_dt, y0=(x0, y0, vx0, vy0), dense_output=True,
                     events=[x.insideEvent for x in LargeList], max_step=3600,
                     t_span=(times[0], times[-1]), # t_eval=times,
                     atol=(1e-3, 1e-3, 1e-3, 1e-3))
    return traj

def marsCapture(traj):
    sepVs = (Mars.orbitPos(times) - traj.sol(times)[:2]).T
    mags = sp.sqrt(inner1d(sepVs, sepVs)) - Mars.rad - 1210e3
    return mags.min()
...
def miniMe(launchAngle):
    ...
    return marsCapture(path)

#res = sp.optimize.minimize(fun=miniMe, x0=0)
Demo2.pos, Demo2.vel = earthRelease(-60)#res.x)
path = pathFinder(*Demo2.pos, *Demo2.vel)
print(marsCapture(path))
#%%
...
if path.status == 1: #temination events
    ...
    tStopI = len(times)
if path.success:
    anim = FuncAnimation(fig, animate, frames=range(1000, len(times[:tStopI])),
