# -*- coding: utf-8 -*-
"""
Created on Tue May 19 14:54:50 2020
...
#%%
TIME_STEP = 5 #time resolution in seconds
...
class Small(PointMass): #variable v, no gravity
    def __init__(self, mass, r, v):
        ...
        self.patch = pat.Arrow(self.r[0], self.r[1], self.v[0], self.v[1], width=6e5) #position at tail of arrow
    ...
    def patchUpdate(self): #ditto, small objects are too small to see so v vector shown
        ...
        self.patch = pat.Arrow(self.r[0], self.r[1], 100*self.v[0], 100*self.v[1], width=6e5) #redraw arrow
...
ax1 = plt.axes(xlim=(-lim, lim), ylim=(-lim, lim) ) #set up first set of axes
A = Large(mass=5e24, r=[0,0], v=[5e2, -1e2], R=6.38e6) #massive bod
B = Small(mass=250, r=[0, 6.8e6], v=[8564, 0]) #petite lil satellite, moving at orbital v
#plt.grid()
...
def init():
    ...
    return ax1.patches #A.patch, B.patch #patches returned as iterable for blitting alg
