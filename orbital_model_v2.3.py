...
def g(Mass, Mass_pos, r): #test grav acceleration at r from mass at rMass
...
class _RigidBody:
    ...
    def __init__(self, mass, pos, rad, pointFmt): #on start:
        ...
        #^ move this line to the animate so ax1 isn't stated before axes are defined

    def pointUpdate(self): # Updates the position of self in the animation
        self.point.set_data(self.pos[0], self.pos[1])
        return self.point

    def contact(self, other): #is centre of other inside radius?
        return True if mag(self.pos, other.pos) <= min(self.rad , other.rad) else False #or me inside you?

    def update(self): #package artists into list to extend updated list
        return [self.patchUpdate(), self.pointUpdate()]

    def insideEvent(self, t, U): #convenience func
...
def gSum(t, r):
    allAcels = sum( g(body.mass, body.orbitPos(t), r) for body in LargeList )
    #print(t, allForces)
    return allAcels
...
vel_text = ax2.text(0.02, 0.95, '', transform=ax2.transAxes)
...
if path.success:
    anim = FuncAnimation(fig, animate, frames=len(t[:tStopIndex]),
                         ...
                         repeat=False, blit=True) #Vary interval, add legend of interval 
    fig.show()

#Add legend that follows each point to give speed of each particle  (realtive to nearest Large body?)
