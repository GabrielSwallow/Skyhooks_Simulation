...
class Small(RigidBody): #variable v, no gravity
    ...
    def coalesce(self, other): #useless function to define new arguments
        ...
        return Small(mass, pos, vel, rad, self.angle, angV, pointFmt="ok") #returns class
...
def init():
    ...
    focus(Demo2)
    return (Sun.point, Earth.point, Mars.point, Demo2.point, Demo2.pointB, *ax2.patches)

def animate(frame):
    updated = [Sun.point, Sun.patch]
    for planet in LargeList[1:]:
        planet.pos = planet.orbitPos(t[frame])
        updated.extend(planet.update())
    Demo2.a = sum(body.gravFS(Demo2.pos) for body in LargeList)
    Demo2.vel = Demo2.vel + Demo2.a * TIME_STEP
    Demo2.move()
    updated.extend(Demo2.update())
    Demo2.pointB.set_data(*path.sol(frame)[:2])
    updated.append(Demo2.pointB)
    focus(Demo2)
